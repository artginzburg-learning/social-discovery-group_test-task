import { calculateLeaderboardPlaces } from './calculateLeaderboardPlaces.js';
...
console.log('Attempt:', new Date().toString());
...
function checkResult(answer, correctAnswer) {
  ...
  for (let i = 0; i < correctAnswer.length; i++) {
    ...
    const answerElement = answer.find((x) => x.userId === correctAnswerElement.userId);
    if (!answerElement) return false;

    if (String(answerElement.place) !== String(correctAnswerElement.place)) return false;
  }
